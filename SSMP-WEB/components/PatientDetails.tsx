...
import { Patient, SurveyStatus, Procedure, ScriptStage, StageData, ClinicalNote } from '../types';
...
const PatientDetails: React.FC<PatientDetailsProps> = ({ patient, procedures = [], onBack, onEdit, onNewProtocol }) => {
  ...
  useEffect(() => {
    const loadTreatments = async () => {
      try {
        ...
        setTreatments(data);
        if (data.length > 0) {
          // Default to most recent (first in list due to order)
          setActiveTreatment(data[0]);
        }
      } catch (error) {
            ...
            <div className="flex flex-col">
              ...
              <div className="flex gap-2 mt-3 flex-wrap">
                <div className="flex h-7 shrink-0 items-center justify-center gap-x-1.5 rounded-full bg-primary/10 px-3 border border-primary/20">
                  ...
                  <p className="text-primary text-xs font-bold">{activeTreatment?.procedureName || patient.procedures.join(', ')}</p>
                </div>
              </div>
            </div>
          </div>
