...
const CRMQuizSettings: React.FC = () => {
    ...
    const handleSave = async () => {
        try {
            setSaving(true);
            await supabaseService.saveQuizConfig(questions, finalScreen);
    ...
    return (
        <div className="space-y-6 h-full overflow-y-auto pr-2 pb-20">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-[#2d181e] p-6 rounded-xl border border-gray-200 dark:border-primary/10 shadow-sm sticky top-0 z-10">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Editor do Quiz</h1>
                    <p className="text-gray-600 dark:text-gray-400 mt-1">Personalize as perguntas e a tela final do seu formulário.</p>
                </div>
                <button
                    onClick={handleSave}
                    disabled={saving}
